...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier * w.Weight) + (math.Pow(speedMps, 2)/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	distanceMeters := float64(s.LengthPool * s.CountPool)
	return (distanceMeters / MInKm) / s.Duration.Hours()
}
